...
const { createSessionStore } = require("./src/store/session-store");
const { createPtyManager, defaultTransportForProvider } = require("./src/runtime/pty-manager");
const {
  ...
  networkUrls,
  resolveCommand
} = require("./src/runtime/cli-helpers");
const { ensureNodePtySpawnHelper } = require("./src/runtime/ensure-node-pty");
const { listSessionRecords, removeSessionRecord } = require("./src/runtime/session-registry");
...
async function main() {
  ...
  if (action === "print-claude-hooks") {
    ...
    const settings = printClaudeHooksConfig({
      serverUrl: server,
      handlerPath: path.resolve(__dirname, "server.js")
    });
