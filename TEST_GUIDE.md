...
本文档描述如何验证 Workshop 和 Terminal 界面重新设计的正确性。测试通过 Chrome DevTools MCP 工具进行截图和交互验证。测试模型应自行记录每步结果，最终给出通过/不通过的判定。
...
服务启动后会输出类似：
```
AgentTown listening on http://127.0.0.1:8765
```
...
| 页面 | URL | 说明 |
|------|-----|------|
| Workshop（主页） | `http://127.0.0.1:8765/#/` | Worker 状态总览、Quick Launch |
...
页面左上角应有品牌标记（铁锈色圆角方块 + globe SVG 图标）、"AGENTTOWN" eyebrow 小字、"Workshop" 标题使用 DM Serif Display 衬线字体。右上角有 `N active` pill 和带绿色圆点的 `online` 状态 pill。

主体为两列布局：左侧 Workshop Floor 面板，右侧 Quick Launch 面板。面板使用 `#fffbf5` 奶白背景、`1px` 边框（不是粗 3px）、`20px` 圆角、柔和投影。

Workshop Floor 内有 2×2 网格的四个 zone：Workshop Floor（淡紫）、Approval Desk（暖橙）、Attention Desk（粉红）、Idle（米色）。Zone 使用降低饱和度的颜色背景 + `1px` 边框。有 worker 的 zone 显示 provider 图标 + 标题 + 状态的卡片；空 zone 显示虚线边框 "No workers here."。
...
关键指标：Workshop Floor 面板 + 四个 zone + Quick Launch 按钮全部在首屏可见，用户无需滚动即可看到 Launch 按钮。
...
顶部 topbar：左侧 "← Workshop" 返回按钮 + "TERMINAL" eyebrow + 会话标题（DM Serif Display 字体）+ "provider · state" 副标题；右侧状态 pill。Topbar 有 `1px` 底部边框和毛玻璃背景效果。
...
- 如果是 tmux-backed 会话（通过 `agenttown claude` 启动），终端应显示 Claude Code 界面，内容实时渲染
- 终端可交互：通过 MCP `type_text` 或 `press_key` 输入文字后终端应有响应
- 如果是 hook-only 会话（没有 tmux），终端应显示 `[terminal unavailable]` 提示
- 点击 "← Workshop" 返回按钮，页面应回到 Workshop 主页
...
点击 Launch Claude 按钮后，按钮应短暂变为 disabled。几秒后 Workshop 中某个 zone 应出现新的 "Claude Session" worker 卡片。点击该卡片应能进入 Terminal 页面并看到 Claude Code 终端。
...
| 视口宽度 | Workshop 布局 | Terminal 布局 |
|----------|--------------|--------------|
| >1080px | 两列（Workshop Floor + Quick Launch 并排） | 左右（终端 + sidebar 并排） |
...
# 清理 agenttown 的 tmux sessions
tmux list-sessions 2>/dev/null | grep agenttown | cut -d: -f1 | xargs -I{} tmux kill-session -t {}
```
