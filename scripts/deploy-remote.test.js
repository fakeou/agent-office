...
test("remote deploy script exists and covers the production deployment flow", () => {
  ...
  assert.match(script, /git clean -fdx -e apps\/app\/dist -e apps\/app\/dist-prev -e apps\/app\/dist-deploy/);
  assert.match(script, /pnpm install --frozen-lockfile/);
  assert.match(script, /python3\.11/);
  assert.match(script, /npm_config_python/);
  assert.match(script, /pnpm --filter @agent-office\/app exec tsc -b/);
  assert.match(script, /vite build --outDir dist-deploy --emptyOutDir/);
  assert.match(script, /apps\/app\/dist-deploy\/index\.html/);
  assert.match(script, /swap_frontend_dist/);
  assert.match(script, /local live_dir="\$DEPLOY_DIR\/apps\/app\/dist"/);
  assert.match(script, /local next_dir="\$DEPLOY_DIR\/apps\/app\/dist-deploy"/);
  assert.match(script, /local backup_dir="\$DEPLOY_DIR\/apps\/app\/dist-prev"/);
